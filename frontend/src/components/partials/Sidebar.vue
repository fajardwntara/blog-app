<template>
 ...
 <BaseAlert v-model="alert.visible" :type="alert.type" :message="alert.message" :duration="2000" />
    ...
    <div id="sidebar" ref="sidebar"
      class="flex lg:flex! flex-col absolute z-40 left-0 top-0 lg:static lg:left-auto lg:top-auto lg:tranteal-x-0 h-[100dvh] overflow-y-scroll lg:overflow-y-auto no-scrollbar w-64 lg:w-20 lg:sidebar-expanded:!w-64 2xl:w-64! shrink-0 bg-white dark:bg-gray-800 p-4 transition-all duration-200 ease-in-out"
      :class="[variant === 'v2' ? 'border-r border-gray-200 dark:border-gray-700/60' : 'rounded-r-2xl shadow-xs', sidebarOpen ? 'tranteal-x-0' : '-tranteal-x-64']">
      ...
      <div class="flex justify-between mb-10 pr-3 sm:px-2">
        ...
        <router-link class="block" to="/vehicle-management">
          <svg class="fill-teal-700 dark:fill-gray-400" version="1.1" id="Layer_1" xmlns="http://www.w3.org/2000/svg"
      ...
      <div class="space-y-8">
        ...
        <div>
          ...
          <ul class="mt-3">
            ...
            <!-- Vehicles -->
            <router-link to="/menu/post" custom v-slot="{ href, navigate, isExactActive }">
        ...
        <div>
          ...
          <ul class="mt-3">
            ...
            <li class="cursor-pointer pl-4 pr-3 py-2 rounded-lg mb-0.5 last:mb-0 bg-linear-to-r" @click="goToUserPage" 
                :class="isExactActive || isCurrentRoute('/menu/user') && 'from-teal-700/[0.12] dark:from-violet-500/[0.24] to-teal-500/[0.04]'">
                <a class="block text-gray-800 dark:text-gray-100 truncate transition"
                  :class="isExactActive ? '' : 'hover:text-gray-900 dark:hover:text-white'" :href="href"
                  @click="navigate">
                  <div class="flex items-center">
                    ...
                    <span
                      class="text-sm font-medium ml-2 lg:opacity-0 lg:sidebar-expanded:opacity-100 2xl:opacity-100 duration-200">User</span>
                  </div>
                </a>
              </li>
          </ul>
